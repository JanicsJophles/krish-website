...
export const WordleKeyboard = ({ onKeyPress, guesses, solution }: WordleKeyboardProps) => {
  ...
  const getKeyClassName = (key: string) => {
    ...
    const baseClasses = "touch-manipulation select-none flex-1 px-1.5 py-3.5 sm:px-2 sm:py-4 m-0.5 text-base sm:text-base font-bold rounded-md transition-colors duration-200";
    
    const specialKeyClasses = key === 'ENTER' || key === 'BACKSPACE' 
      ? 'min-w-[4.5rem] sm:min-w-[4.5rem]' 
      : 'min-w-[2.25rem] sm:min-w-[2.25rem]';
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full max-w-xl mx-auto px-1 pb-2">
      {rows.map((row, i) => (
